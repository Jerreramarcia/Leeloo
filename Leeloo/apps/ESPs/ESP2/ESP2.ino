...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  Serial.print(topic);
  Serial.print("] ");
  if (strcmp(topic,"casa/habitacion/juanma/led")==0){
    Serial.print("Encendiendo led habitacion juanma");
    ...
    if (strcmp(topic,"casa/habitacion/alicia/led")==0){
      ...
      if ((char)payload[0] == '1') {
        digitalWrite(ALICE, HIGH);   // Turn the LED on (Note that LOW is the voltage level
        // but actually the LED is on; this is because
        // it is acive low on the ESP-01)
      } else {
        digitalWrite(ALICE, LOW);  // Turn the LED off by making the voltage HIGH
      }
    }
}
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client")) {
      ...
      client.subscribe("casa/habitacion/juanma/led");
